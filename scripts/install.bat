...
set "pluginName=PluginUpdater"
set "bmPluginsFolder=%APPDATA%\bakkesmod\bakkesmod\plugins"
set "dllFile=%~dp0%pluginName%.dll"
set "cfgFolder=%APPDATA%\bakkesmod\bakkesmod\cfg"


rem ----------------------------------------------------------------------
echo.
echo Copying %pluginName%.dll to bakkesmod plugins folder...
...
if errorlevel 1 (
    echo Error occurred while copying %pluginName%.dll to bakkesmod plugins folder.
    echo.
    echo.
    pause
    exit /b 1
) else (
    echo %pluginName%.dll successfully copied to "%bmPluginsFolder%"
)


rem ----------------------------------------------------------------------
echo.
echo Adding line: "plugin load %pluginName%" to plugins.cfg...

cd /d %cfgFolder%

set "newLine=plugin load %pluginName%"
